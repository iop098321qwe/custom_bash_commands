...
- `cbc pkg` now writes `[[module.deps]]` entries (`use`, `rev`, `hash`) to
  `~/.config/cbc/packages.toml`. `install` only records metadata, `load` syncs
  modules into `modules/` while refreshing the fields, `update` checks the
  manifest before pulling, and the post-update auto-load must remain intact.
